...
{
    sensor_monitor_t *monitor = (sensor_monitor_t *)args;
    ...
    while (1)
    {
        ...
        if (status == SENSOR_OK)
        {
            ESP_LOGD(SENSOR_MONITOR_TAG, "Sensor %s data read successfully", monitor->sensor_name);
        ...
        {
            ESP_LOGE(SENSOR_MONITOR_TAG, "Failed to read data from sensor %s", monitor->sensor_name);
        }
