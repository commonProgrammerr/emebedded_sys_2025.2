...
{
  /* FIXME: PCF8591_tick() only clears high byte of analog_data, which is redundant
   * since we read 2 bytes and the second byte overwrites it. Consider removing
   * this call or clarifying its purpose. */
...
void PCF8591_set_channel_index(uint8_t channel_index)
{
  ...
  /* FIXME: Variable 'analog_data' is declared but never used. Remove it. */
  uint8_t analog_data[2];
  // Send configuration byte to select the ADC channel
  /* TODO: Add error handling/callback for when HAL_I2C_Master_Transmit_IT fails */
...
{
  /* FIXME: config_byte variable is declared but never used in this function */
  ...
  /* TODO: Check return value of HAL_I2C_Master_Receive_IT and handle errors */
...
void PCF8591_write_dac(uint8_t value)
{
  ...
  // Send configuration byte to select the ADC channel
  /* TODO: Check return value and handle transmission failure */
  ...
  /* FIXME: Single __NOP() is insufficient delay for DAC settling. Either remove
   * (since IT transmission is async) or implement proper delay if needed. */
  __NOP();                     // Small delay to allow ADC to settle
}

// Inicialização
void PCF8591_Init(I2C_HandleTypeDef *hi2c, PCF8591_TickCallback_t tick_callback, PCF8591_TickCallback_t tx_cplt_callback, PCF8591_TickCallback_t rx_cplt_callback)
{
  pcf8591.hi2c = hi2c;
...
/* TODO: Add HAL_I2C_ErrorCallback to handle I2C errors (NACK, timeout, etc.)
 * and update system state accordingly to prevent deadlock. */
